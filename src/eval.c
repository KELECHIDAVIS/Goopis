...
// // eval code from : https://www.chessprogramming.org/PeSTO%27s_Evaluation_Function
// #define PAWN 0
// #define KNIGHT 1
// #define BISHOP 2
// #define ROOK 3
...
// #define EMPTY_PIECE (BLACK_KING + 1)
...
//         if (isValidPiece(board->mailbox[sq])) {

//             U64 pcBit = 1ULL << sq;
//             int color = (getColorPieces(board, nWhite) & pcBit) ? WHITE : BLACK;

//             // translate from my format to pesto's
//             int pc = board->mailbox[sq] - 2;
//             pc = pc * 2 + color;
...
//     side2move = board->whiteToMove ? WHITE : BLACK;
//     /* tapered eval */
...
// TODO: make this quiesience
// returns how good position is for current side to move

//  add material posiition tables to affect weight : https://www.chessprogramming.org/Simplified_Evaluation_Function
short pieceSquareTable[7][64] = {
    {// pawn
     0, 0, 0, 0, 0, 0, 0, 0,
...
double evaluate(const Board *board) {
    ...
    double score = 0;                                      // for rn only counting material score
    int pieceWeights[] = {100, 320, 330, 500, 900, 20000}; // p,n,b,r,q,k

    U64 allPieces = getAllPieces(board);
    while (allPieces) {
        U64 pieceBit = LSBIT(allPieces);
        allPieces = CLEARLSBIT(allPieces);
        enumSquare sq = __builtin_ctzll(pieceBit); 
            enumPiece piece = board->mailbox[sq];
        if (piece == nKing || !isValidPiece(piece))
            continue; // kings will be decided later to see if were currently in late game

        // if white add, if black minus
        int pieceIndx = piece - 2;

        if (getColorPieces(board, nWhite) & pieceBit) {
            score += (pieceWeights[pieceIndx] + pieceSquareTable[pieceIndx][sq]);
        } else {
            score -= (pieceWeights[pieceIndx] + pieceSquareTable[pieceIndx][63 - sq]); // have to flip the table and make it neg for black
        }
    }
